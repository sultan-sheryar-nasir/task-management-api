...
export class UserNotValidException extends HttpException {
    constructor(message: string) {
        super(`Failed to validate user ${message}`, HttpStatus.BAD_REQUEST);
    }
}

export class UserAlreadyTakenException extends HttpException {
    constructor(message: string) {
        super(`Failed to register user ${message}`, HttpStatus.BAD_REQUEST);
    }
}